const ConectorPluginV3 = (() => {
    ...
    class ConectorPlugin {
        ...
        // URL por defecto - Acceso local directo al plugin de impresión
        static URL_PLUGIN_POR_DEFECTO = "http://192.168.88.251:8000";
        ...
        static async obtenerImpresorasRemotas(ruta: string = ConectorPlugin.URL_PLUGIN_POR_DEFECTO, rutaRemota: string = "") {
        ...
        async imprimirEnImpresoraRemota(nombreImpresora: string, rutaRemota: string) {
            ...
            // Construir la URL completa para el reenvío
            const targetUrl = this.ruta + "/reenviar?host=" + rutaRemota;

            // Usar el proxy del backend para evitar problemas de CORS y Mixed Content
            ...
            if (!response.ok) {
                throw new Error(`Error en printer-proxy: ${response.status} ${response.statusText}`);
            }
        ...
        async imprimirEn(nombreImpresora: string) {
          ...
          try {
              ...
              const targetUrl = this.ruta + "/imprimir";
